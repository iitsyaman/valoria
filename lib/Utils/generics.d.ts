...
import { ValoriaEventEmitter, ValoriaEventMap, BrowsersMap, WACallUpdateType, WAVersion } from '../Types';
...
export declare function bindWaitForEvent<T extends keyof ValoriaEventMap>(ev: ValoriaEventEmitter, event: T): (check: (u: ValoriaEventMap[T]) => Promise<boolean | undefined>, timeoutMs?: number) => Promise<void>;
export declare const bindWaitForConnectionUpdate: (ev: ValoriaEventEmitter) => (check: (u: Partial<import("../Types").ConnectionState>) => Promise<boolean | undefined>, timeoutMs?: number) => Promise<void>;
export declare const printQRIfNecessaryListener: (ev: ValoriaEventEmitter, logger: ILogger) => void;
/**
 * utility that fetches latest Valoria version from the main branch.
 * Use to ensure your WA connection is always on the latest version
 */
export declare const fetchLatestValoriaVersion: (options?: AxiosRequestConfig<any>) => Promise<{
    version: WAVersion;
    isLatest: boolean;
    error?: undefined;
} | {
...
/**
 * Stream errors generally provide a reason, map that to a Valoria DisconnectReason
